        ...
        if(entries && (entries.forEach !== undefined) && (entries.length > 0))
        {
            ...
            entries.forEach(channel => {
                ...
                if((typeof channel === 'string') && (channel.length > 0))
                {
                    ...
                    fluidsync.publish({channel: eventChannel, from: proxyName, payload: wrappedMessage});                                
                }
            });
...
function notifySubscribersOnPubsubEvent(token, dictionary, message)
{
    ...
    if(subscribers !== undefined)
    {
        subscribers.forEach(eventChannel => {
            ...
            fluidsync.publish({channel: eventChannel, from: proxyName, payload: message});      
        });        
    }
}
...
function onPresenceTick()
{
    if(fluidsyncSocketId !== undefined)
    {
        ...
        fluidsync.publish({channel: proxyPresenceChannel, from: proxyName, payload: wrappedMessage}); 
    }
}
...
fluidsync.addEventListener('close', (fluidsync, code, reason) => {
    ...
    console.log('disconnected from FluidSync: code=' + code + ', reason=' + reason);            
});
...
function sendReply(message, err, reply)
{
    ...
    if((typeof feedbackChannel === 'string') && (feedbackChannel.length > 0))
    {
        ...
        fluidsync.publish({channel: feedbackChannel, from: proxyName, payload: payload});            
    }
}
